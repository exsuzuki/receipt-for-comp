...
var RATE;
(function (RATE) {
    RATE[RATE["TAX"] = 0.1] = "TAX";
    RATE[RATE["Adjustment"] = 0.0196] = "Adjustment";
})(RATE || (RATE = {}));
...
function btn_click() {
    BasePrice = Number(document.getElementById('inputPrice').value);
    console.log(BasePrice);
    if (isRegistration)
        Adjustment = 0;
    else
        Adjustment = Math.floor(BasePrice * RATE.Adjustment);
    ExcludingTAX = BasePrice - Adjustment;
    TAX = Math.floor(ExcludingTAX * RATE.TAX);
    IncludingTAX = ExcludingTAX + TAX;
    document.getElementById('BasePrice').innerHTML = BasePrice.toLocaleString();
    document.getElementById('Adjustment').innerHTML = Adjustment.toLocaleString();
    document.getElementById('ExcludingTAX').innerHTML = ExcludingTAX.toLocaleString();
    document.getElementById('TAX').innerHTML = TAX.toLocaleString();
    document.getElementById('IncludingTAX').innerHTML = IncludingTAX.toLocaleString();
    drawReceipt();
}
function drawReceipt() {
    ...
    let AdjustmentText = '';
    if (!isRegistration)
        AdjustmentText = ' 経過措置控除:' + Adjustment.toLocaleString();
    ctx.font = '9px san-serif';
    ctx.textAlign = 'center';
    ctx.fillText('但 〇〇料として' + AdjustmentText, cvs.width / 2, 82.5);
    // 印紙
    ctx.font = '8px san-serif';
    ctx.textAlign = 'center';
